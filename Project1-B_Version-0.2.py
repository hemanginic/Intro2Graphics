...
Revision History: 
Project1-B Version 0.1 - Adding evilfishsprites and poisonivy sprites (poisonivy sprite is not working proper).
Project1-A Version 0.2 - All the sprites are working properly. the speed of evilsprite and poisonivy sprite changed.
                         splash screen added.Score calculation fixed.
...
class Moustache(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("evilfish.gif")
        self.rect = self.image.get_rect()
        self.reset()
...
def end(score):
    ...
    #Display instructions and print it on the screen
    allSprites = pygame.sprite.Group(water, userfish)
    insFont = pygame.font.SysFont('helvetica',30)
    insLabels = []
    instructions = (
...
#Display splash screen before game starts  
def splashScreen():
    pygame.display.set_caption("Lunch Time!")
    background = pygame.Surface(screen.get_size())
    #background = background.convert()
    background.fill((255, 0, 0))
    logo = pygame.image.load('zooyo.png')
    
    screen.blit(logo, (380,240))
    time.sleep(2)
...
def main():
    #if user want to play game or quit
    ...
    while not donePlaying:
        donePlaying, difficulty = instructions()
